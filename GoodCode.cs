public class OrderProcessor
{
    ...
    public async Task<OrderResult> ProcessOrder(Order order)
    {
        ...
        if (!await InventoryHasRequiredItems(order))
        {
        ...
        catch (Exception ex)
        {
            ...
            LogOrderProcessingFailure(ex);
            throw;
        }
    }

    private void EnsureOrderIsProvided(Order order)
    {
        if (order == null)
        {
    ...
    private async Task<bool> InventoryHasRequiredItems(Order order)
    {
    ...
    private void LogOrderProcessingFailure(Exception ex)
    {
    ...
    public async Task CancelOrder(string orderId)
    {
        ...
        {
            await RefundAndRestockOrder(order);
        }

        await MarkOrderAsCancelled(order);
    }

    private async Task RefundAndRestockOrder(Order order)
    {
...
// Clear, domain-specific method names remove the need for 
// explanatory comments and make the code self-documenting, 
// aligning with Clean Code’s guidance to express intent through 
